...
// TestDefaultOilPlantConfigParseable verifies the actual firewall config
// file can be loaded and parsed into the expected structures.
func TestDefaultOilPlantConfigParseable(t *testing.T) {
	// Try to find the config file relative to the test location
	paths := []string{
		"../../../lab-definitions/firewall/default-oil-plant.json",
		"lab-definitions/firewall/default-oil-plant.json",
	}
	...
	if data == nil {
		t.Skip("default-oil-plant.json not found (run from repo root)")
	}
	...
	if config.Firewall.DefaultAction != "DENY" {
		t.Errorf("expected default action DENY, got %s", config.Firewall.DefaultAction)
	}
	if len(config.Firewall.Rules) == 0 {
		t.Error("expected at least one firewall rule")
	}

	// Verify critical rules exist
	ruleIDs := make(map[string]bool)
	for _, r := range config.Firewall.Rules {
		ruleIDs[r.ID] = true
	}

	criticalRules := []string{
		"it-to-dmz-standard",
		"deny-write-to-safety",
		"hmi-view-to-ot-control-modbus-ro",
		"hmi-view-to-ot-safety-modbus-ro",
	}
	for _, id := range criticalRules {
		if !ruleIDs[id] {
			t.Errorf("missing critical rule: %s", id)
		}
	}

	// Verify safety zone deny rule blocks write function codes
	for _, r := range config.Firewall.Rules {
		if r.ID == "deny-write-to-safety" {
			if r.Action != "DENY" {
				t.Errorf("deny-write-to-safety should be DENY, got %s", r.Action)
			}
			if r.ICS == nil {
				t.Fatal("deny-write-to-safety should have ICS config")
			}
			// FC 5,6,15,16,22,23 are Modbus write function codes
			writeFCs := map[int]bool{5: true, 6: true, 15: true, 16: true, 22: true, 23: true}
			for _, fc := range r.ICS.FunctionCodes {
				if !writeFCs[fc] {
					t.Errorf("unexpected function code %d in deny-write-to-safety", fc)
				}
			}
		}
	}
...
// TestDefaultOilPlantConfigZones verifies the firewall config has the
// expected zone structure for OT segmentation.
func TestDefaultOilPlantConfigZones(t *testing.T) {
	paths := []string{
		"../../../lab-definitions/firewall/default-oil-plant.json",
		"lab-definitions/firewall/default-oil-plant.json",
	}
	...
	if data == nil {
		t.Skip("default-oil-plant.json not found")
	}
	...
	expectedZones := map[string]bool{"wan": false, "dmz": false, "lan1": false, "lan2": false}
	for _, z := range config.Zones {
		if _, ok := expectedZones[z.Name]; ok {
			expectedZones[z.Name] = true
		}
	}
	for name, found := range expectedZones {
		if !found {
			t.Errorf("missing required zone: %s", name)
		}
	}

	// Verify interface-to-zone mapping
	expectedIfaces := map[string]string{"eth0": "wan", "eth1": "dmz", "eth2": "lan1", "eth3": "lan2"}
	ifaceZones := make(map[string]string)
	for _, iface := range config.Interfaces {
		ifaceZones[iface.Device] = iface.Zone
	}
	for dev, zone := range expectedIfaces {
		if ifaceZones[dev] != zone {
			t.Errorf("expected %s -> %s, got %s -> %s", dev, zone, dev, ifaceZones[dev])
		}
	}
}
