...
- `/api/containd/*` → containd firewall API

`NEXT_PUBLIC_API_URL` is set to `/api` at build time so axios/fetch
calls land on the proxy, not on `:3000`.
...
CI fails on any ESLint error. The repo policy is no-`any` and no
unused-variable suppressions; if you find yourself reaching for one,
push back on the design first.
